...
## Basic workflow
1.Rawdata  
2.Sample Pheno & batch data 
3.Check for duplicate Study Id
4.Match Sample.ID (Chips) to Sample data provided in 
...
## 7. Write Expression data files to project directory for eset_raw
...
Standard QC plots, PCA vs Batch variables and Sample Network stats
```{r basicQCplots_eset_raw, fig.width=16, fig.height=8, eval=TRUE}
# Basic QC plots & Sample Network Plot based an all data pre-qc
gx_qc_plots_lumi_2(eset_raw, do_pca=TRUE)
```
...
-------------------------------------------------------
```{r checkGender, fig.width = 10, fig.height = 10,eval=TRUE}
cat(" Checking Gender based on XIST gene expression","\r","\n")

## get neg control data from eset_bg
cat(" get negativeControl data from eset_bg","\r","\n")
...
## cobine xist_gx_raw and xist_gx_bkcor data
...
xist_gx$clinical_gender <- toupper(pData(eset_bg)$SEX)
...
xist_gx$xist_p <- ifelse(xist_gx$XIST_z >= 0, 2*pnorm(-abs(xist_gx$XIST_z)) ,1 )
# head
head(xist_gx)
...
#boxplot( log2(bkcor_XIST) ~ xist_illumina_detection_p_gender, data=xist_gx, main="bg XIST.gx ~ xist_illumina_detection_p_gender")
plot(log2(xist_gx$bkcor_XIST), type="p", col=labels2colors(xist_gx$xist_gender), pch=19 , cex=0.6);
points(log2(xist_gx$neg_2sd),col="grey", type="l")
points(log2(xist_gx$neg_mean),col="black", type="l")
points(log2(xist_gx$neg_min),col="lightgrey", type="l")
points(log2(xist_gx$bkcor_XIST), type="p", col=labels2colors(xist_gx$xist_gender), pch=19 , cex=0.6);
#plot(xist_gx$bkcor_XIST, type="p", col=labels2colors(xist_gx$xist_gender), pch=19 , cex=0.6);
#points(xist_gx$neg_2sd,col="blue", type="l")
#points(xist_gx$neg_mean,col="black", type="l")
#dev.off()
...
## Save Transformed and Normalised data pre-sample removal
...
## QC Plots Transformed and Normalised data
```{r qc_plot_eset_bg_log2_rsn_0, eval=TRUE}
# Basic Plots
...
## Write Expression data files for Transformed and Normalised data
...
#
ISAoutliers_file <- paste(out_dir,"/",project_name,".eset_bg_log2_rsn_0.group.basic_sampleNetworkIterate_summary.csv",sep="")
...
------------------------------------------------------------------
```{r qcPlots_eset_bg_log2_rsn_post_sampleNetwork,  eval=TRUE}
...
```{r gx_qc_plots_lumi_eset_bg_log2_rsn_post_sampleNetwork, eval=TRUE}
# QC Plots of Transformed and Normalised data 
...
project_summary <- data.frame(
  ...
  lab_contact_email=lab_contact_email,
  lab_contact_name=lab_contact_name,
  n_samples=dim(eset_raw)[2],
  n_samples_final=dim(eset_bg_log2_rsn)[2],
  n_samples_dupe=dim(dupe_samples)[1],
  n_xist_gender_fails=n_gender_fails,
  n_outliers=length(outlier_samples),
  n_probes=dim(eset_raw)[1],
  n_probes_detected=dim(eset_final)[1],
...
system( paste(" mkdir ",out_dir,"/SampleNetwork_out; mv -v ",out_dir,"/*.SampleNetwork* ",out_dir,"/SampleNetwork_out/",sep="") );
system( paste(" mv -v ",out_dir,"/*sampleNetworkIterate* ",out_dir,"/SampleNetwork_out/",sep="") );

# clean up linear model output
system( paste(" mkdir ",out_dir,"/batch_regresions; mv -v ",out_dir,"/*multivariate_model* ",out_dir,"/batch_regresions/",sep="") )

# clean up plots
system( paste(" mkdir ",out_dir,"/plots; mv -v ",out_dir,"/*.pdf ",out_dir,"/plots/",sep="") )

# clean up data for final_data (END) & raw (START)
system( paste(" mkdir ",out_dir,"/raw_data; cp -v ",out_dir,"/*eset_raw* ",out_dir,"/raw_data/",sep="") )
system( paste(" mkdir ",out_dir,"/final_data; cp -v ",out_dir,"/*eset_final* ",out_dir,"/final_data/",sep="") )
