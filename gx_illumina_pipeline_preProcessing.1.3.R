...
lumi.NQ <- lumiExpresso(eset_raw, variance.stabilize=TRUE,  varianceStabilize.param=list(method='log2'),  normalize.param=list(method='quantile'), bg.correct=FALSE)
...
outsnq <- basic_sampleNetworkIterate(eset=lumi.NQ,col_by_chip=0,groups="byGroup",outfile=paste(out_dir,"/",project_name,".lumi.NQ",sep=""),IACthresh=0.95, sd_thrs=2 );
