...
data_summary_plots(data,plotOut)
...
write.table(FNC_DF,file=paste(outfile,".group.",group,".SampleNetwork_Stats.txt",sep=""),sep="\t",row.name=FALSE,quote=FALSE)
## write data for IAC 
cat(" Saving Data fram of fundamentalNetworkConcepts Z.K outliers [",paste(outfile,".SampleNetwork_Stats_Z.K_outliers.txt",sep=""),"]","\r","\n")
write.table(FNC_DF[Z.K_outliers,],file=paste(outfile,".group.",group,".SampleNetwork_Stats_Z.K_outliers.txt",sep=""),sep="\t",row.name=FALSE,quote=FALSE)
...
## Cluster for pics
cat(" Plotting SampleNetwork Metrics [",paste(outfile,".group.",group,".SampleNetwork.qc.pdf",sep=""),"]","\r","\n")
...
## PLOTS 
## cluster IAC
pdf(file=paste(outfile,".group.",group,".SampleNetwork.qc.pdf",sep=""),width=11,height=8)
par(mfrow=c(2,2))
par(mar=c(5,6,4,2))
...
res[group,] <- c(group,mean_IAC,n_outliers,min_Z.K,rho,rho_pvalue,Z.K_outliers)
...
cat(" Groups [",group_list,"]","\r","\n")
	...
	write.table(FNC_DF[Z.K_outliers,],file=paste(outfile,".group.",group,".SampleNetwork_Stats_Z.K_outliers.txt",sep=""),sep="\t",row.name=FALSE,quote=FALSE)
	## set colours by chip of GROUPS
	if(col_by_chip == 1) {   colorvec <- labels2colors(as.character(pData(eset[,pData(eset)$GROUPS==group])$Sentrix.Barcode)) }
	if(col_by_chip == 0) {   colorvec <- gpcolors }
	## plots of fundamentalNetworkConcepts
	local({
	colLab <<- function(n,treeorder) {
	...
	Z.K_outliers <- paste(Z.K_outliers, collapse=";")
	      ...
	      ## FINAL RESULTS 
	res_out <- as.data.frame(res)
	save(res_out,fie=paste(outfile,".group.",mgroup,".SampleNetwork.qc.RData",sep=""))
	return(res_out)
	      }
}
...
n_groups <- 1
basic_sampleNetworkIterate_summary_names <- c("Group","round","nSamp","nOutlier","mean_IAC","n_outliers","min_Z.K","rho","rho_pvalue","Z.K_outliers")
basic_sampleNetworkIterate_summary <- matrix(ncol=length(basic_sampleNetworkIterate_summary_names),nrow=n_groups, dimnames=list(group_list,basic_sampleNetworkIterate_summary_names))

group <- "all"

out <- basic_sampleNetwork(eset,col_by_chip,groups,outfile=paste(outfile,".group.",group,".round.",iteration,sep="" ))
...
Z.K_outliers_list <- strsplit(Z.K_outliers_list,";")
...
if(outlier_running_count == 0 ) { write.table(basic_sampleNetworkIterate_summary,file=paste(outfile,".group.",group,".basic_sampleNetworkIterate_summary.csv",sep=""),sep=",",row.names=FALSE,quote=FALSE) } 
...
while(iteration >= 1 &  min_Z.K < -sd_thrs) {
...
write.table(iac_outlier_samples_df,file=paste(outfile,".group.",group,".iac_outlier_samples.txt",sep=""),sep="\t",row.names=FALSE,quote=FALSE)
write.table(basic_sampleNetworkIterate_summary,file=paste(outfile,".group.",group,".basic_sampleNetworkIterate_summary.csv",sep=""),sep=",",row.names=FALSE,quote=FALSE)
...
out <- basic_sampleNetwork(eset[,pData(eset)$GROUPS==mgroup],col_by_chip=0,groups="all",outfile=paste(outfile,".group.",mgroup,".round.",iteration,sep="" ))

n_samp <-  length(sampleNames(eset[,pData(eset)$GROUPS==mgroup]))
mgroup <- out$Group
mean_IAC <- out$mean_IAC;
min_Z.K <- out$min_Z.K;
rho_pvalue <- out$rho_pvalue
...
if(outlier_running_count == 0 ) { write.table(basic_sampleNetworkIterate_summary,file=paste(outfile,".basic_sampleNetworkIterate_summary.csv",sep=""),sep=",",row.names=FALSE,quote=FALSE) } 
...
out <- basic_sampleNetwork(eset[,pData(eset)$GROUPS==mgroup],col_by_chip,groups,outfile=paste(outfile,".group.",mgroup,".round.",iteration,sep="" ))
...
write.table(basic_sampleNetworkIterate_summary,file=paste(outfile,".group.",mgroup,".basic_sampleNetworkIterate_summary.csv",sep=""),sep=",",row.names=FALSE,quote=FALSE)
...
outs <- basic_sampleNetworkIterate(eset=eset_raw,col_by_chip=0,groups="byGroup",outfile=paste(out_dir,"/",project_name,".XXXXeset_raw",sep=""),IACthresh=0.95, sd_thrs=2 )
